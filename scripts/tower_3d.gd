...
func _rebuild_body() -> void:
	...
	match data.fire_mode:
		"radial":
			...
			# 20% larger than the footprint-filling size (per request); inner/outer
			# scaled together to keep the dialed-in opening ratio.
			var inner := r * 0.379
			var outer := r * 1.08
		...
		_:
			_part(_low_poly_cylinder(r * 0.9, 40.5, 8), mat, 0.0)
