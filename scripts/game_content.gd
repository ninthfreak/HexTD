...
func _tower_from_dict(d: Dictionary) -> TowerData:
	...
	t.rule_charges = maxi(1, int(d.get("rule_charges", 4)))
	t.max_rules = maxi(1, int(d.get("max_rules", 6)))
	...
	t.execute_no_decay = bool(d.get("execute_no_decay", false))
	t.cipher = bool(d.get("cipher", false))
	t.buffer_overflow = bool(d.get("buffer_overflow", false))
	t.dos = bool(d.get("dos", false))
	t.dos_freeze = maxf(0.0, float(d.get("dos_freeze", 0.5)))
	t.dos_slow_time = maxf(0.0, float(d.get("dos_slow_time", 2.0)))
	t.dos_slow_factor = clampf(float(d.get("dos_slow_factor", 0.5)), 0.05, 1.0)
	...
	for slot_raw in arr:
		...
		if typeof(tiers_raw) == TYPE_ARRAY:
			for u in tiers_raw:
				...
				for stat in ["damage", "range", "fire_rate", "directions", "targets", "hops", "hop_range", "hop_falloff", "arc_angle", "rule_charges", "max_rules", "ramp_time", "focus_time", "charge_retain", "dos_freeze", "dos_slow_time", "dos_slow_factor", "ecc_pierce", "execute_threshold", "height", "width"]:
					if u.has(stat):
